"""Public benchmark pipeline stage: H3 observed event panel generation.
...
REQUIRED_INPUTS = ['data/processed/fase3_finales_iccs/cdmx_final_iccs.parquet']


def main() -> int:
    missing = [p for p in REQUIRED_INPUTS if not Path(p).exists()]
    if missing:
        print("03_generate_h3_panel.py: raw/intermediate inputs are not available in this public package.")
        ...
        for item in missing:
            print(f"  - {item}")
        return 2
    print("03_generate_h3_panel.py: required inputs found. Insert project-specific execution here or run the audited notebooks/scripts from the full thesis workspace.")
    return 0
